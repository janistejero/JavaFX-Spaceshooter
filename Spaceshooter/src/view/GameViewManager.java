...
public class GameViewManager {
    ...
    private void checkIfElementsAreBehindTheShipAndRelocate() {
        ...
        for (int i = 0; i < brownMeteors.length; i++) {
            if (brownMeteors[i].getLayoutY() > 2000) {
                setNewElementPosition(brownMeteors[i]);
            }
        }

        for (int i = 0; i < greyMeteors.length; i++) {
            if (greyMeteors[i].getLayoutY() > 2000) {
                setNewElementPosition(greyMeteors[i]);
            }
        }
    }
    ...
    private void checkIfElementsCollide(){
        ...
        for (int i = 0; i < brownMeteors.length; i++) {
             if(METEOR_RADIUS + STAR_RADIUS > calculateDistance(ship.getLayoutX() + 49, brownMeteors[i].getLayoutX() + 20, ship.getLayoutY() + 37, brownMeteors[i].getLayoutY() + 20)){
                 removeLife();
                 setNewElementPosition(brownMeteors[i]);
             }
        }
        ...
        for (int i = 0; i < greyMeteors.length; i++) {
             if(METEOR_RADIUS + STAR_RADIUS > calculateDistance(ship.getLayoutX() + 49, greyMeteors[i].getLayoutX() + 20, ship.getLayoutY() + 37, greyMeteors[i].getLayoutY() + 20)){
                 removeLife();
                 setNewElementPosition(greyMeteors[i]);
             }
        }
